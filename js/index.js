...
function Surface() {
  var now = new Date();
  this.accent = window.getComputedStyle(
    document.body.querySelector(".day")
  ).color;
  this.nightAccent = window.getComputedStyle(
    document.body.querySelector(".night")
  ).color;
  this.nightStart = 18;
  this.fontWeight = 300;
  this.fontFamily = window
    .getComputedStyle(document.querySelector(".container"))
    .fontFamily.split(/\,\s/)[0]
    .replace(/[\"|\']/g, "");
  this.nightEnd = 8;
  ...
  this.isNight = function() {
    var now = new Date();
    return now.getHours() < this.nightEnd || now.getHours() >= this.nightStart;
  };
  ...
  this.draw = function() {
    ...
    var now = new Now();
    var displayText = [now.hoursText(), now.minsText(), now.secsText()].join(
      "·"
    );
    ...
    this.context.beginPath();
    this.context.arc(0, 0, this.runit() * 2.5, 0, Math.PI * 2);
    this.context.fill();
    ...
    for (var minb = 0; minb < 60; minb++) {
      if (minb % 5 !== 0) {
        if (minb === now.now.getMinutes() || minb === now.now.getSeconds()) {
          this.context.fillStyle = this.nightMode
            ? this.nightAccent
            : this.accent;
        } else {
    ...
    this.context.fillText(displayText, 0, 0 + this.fontSize() / 3);

    if (this.gauges) {
      var digits = parseInt(this.fontSize() / 30);
      this.context.font =
        this.fontWeight + " " + this.fontSize() / 3 + "px " + this.fontFamily;
...
function Now() {
  ...
  this.minsPrcnt = function() {
    return this.now.getSeconds() / 60 + this.now.getMilliseconds() / 1000 / 60;
  };
  this.secsPrcnt = function() {
    return this.now.getMilliseconds() / 1000;
  };
  return this;
}
...
setTimeout(function() {
  if (
    ...
    document.body.querySelector(".container").offsetTop
  ) {
    showHelp();
